...
TEST(TestPath, ValidateValidates) {
	...
	// SprawdŸ czy wêz³y nie powtarzaj¹ siê
	{
		Path p = generateCorrectPath(am);
	...
	// SprawdŸ poprawnoœæ startingPoint
	{
		//Path p = generateCorrectPath(am);
	...
	// SprawdŸ czy wartoœci from odpowiadaj¹ wartoœciom to
	{
		Path p = Path(am);
	...
	// SprawdŸ czy ostatni element to startingPoint
	{
		Path path = Path(am);
		path.InsertEdge(PathEdge(0, 1));
		path.InsertEdge(PathEdge(1, 3));
